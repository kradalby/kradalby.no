...
### Uninformed search
Method have access only to the problem definitions
...
### A\*
A\* is a best first search algorithm that uses a admissible heuristic estimate to choose the best possible node for every iteration.
...
### Fully observable 
The sensors give the again aceess to the complete state of the environment at each point in time.
...
### Deterministic
The next state is only dependant on the current state and the agents action.

Any action will have signle guaranteed effect. There is _no_ uncertainty.

#### Stratigic
Stratigic is a subset of a Deterministic environment, this is because the only uncertainty is the actions of other agents.

### Stochastic
There is some uncertainty about the outcome of an agent action. 

It is harder to create a agent for non-deterministic environments.
...
The action in one episode is not dependant on any other episode than the current.

### Sequential
In a sequential environment all the current decisions can effect the future decisions. This is much harder than episodic, because we do not need to think ahead.
...
### Static
You can assume that the environment does not change inbetween agent actions. Time is not a thing the agent need to care about.

### Dynamic
Dynamic environments can change while the agent is thinking, this requires quick decision making from the agent.
Changes can be done that the agent cant control.

### Known 
The outcomes for all actions are given. Agents knows all the rules like gravity, but the environment can be partially observable if the sensors are not properly working.
...
This agent is the most basic of them all, it will only do checks in its current state and dont care about nothing else.
...
This agent can handle a partially obserable environment. The current state of the world is stored so it can maintain some kind of description of the world which it cannot see. This can be looked at as a model of "how the world works". This information if maintained between actions and updated.

The agent function chooses an action dependendt on the current and some past information.

### Goal-based agents

The goal-based agents further improve the model-based agents by adding goal information. Goal information describes situations which are good. This allows the agent to choose a goal state instad of a non-goal state if there is multiple available posibillities.

### Utility-based agents
This agent further extends the goal-based agent by adding a utility function which can evaluate how good a state is. This makes it possible to always choose the best state we possibly can. It is often refered to as evaluating how happy we will be in the next state.

### Learning agents
All the agents above can be implemented as learning agents. A Learning agent can use information it gathers in an environment. This allows them to intitially operate in an unknown environment.

A learning agent has four elements, Critic, Learning element, Problem generator and Performance element. The learning element is responsible for making improvments and the performance element is responsible for making the actions. The learning element uses the critic to determine how the performance element should be modified to do better in the future. The performance element is what we in previous agents looked at as the whole function. The last element, the problem generator is responsible for suggestion actions that will lead to new and informative experiences.
...
### Inference
Inference is the process of arriving at a logical conclusion based on given premises that you know or assume to be true.
